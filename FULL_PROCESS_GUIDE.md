...
Этот файл описывает весь процесс создания модели `point cloud blob` в Blender: от идеи до готового `.blend` и рендера.

## Цель

Сделать объект, похожий на референс:

- черный фон
- объемная деформированная форма
- поверхность не сплошная, а собрана из светящихся точек
- градиент по высоте: сверху голубой, снизу розово-фиолетовый
...
1. Создать чистую сцену Blender из Python.
2. Добавить базовую сферу, которая служит исходной формой.
3. Через Geometry Nodes деформировать эту сферу шумом.
4. Преобразовать вершины деформированной формы в точки.
5. Заинстансить на каждую точку маленькую `Ico Sphere`.
6. Назначить инстансам emissive-материал.
7. Настроить камеру, черный фон и Eevee Bloom.
8. Сохранить `.blend` и, при необходимости, отрендерить PNG.

## Структура итоговой модели в Blender

В итоговой сцене есть такие объекты:

- `BlobSurface`  
  Основной объект. Это UV Sphere, на которую навешан Geometry Nodes modifier.

- `DotInstance`  
  Маленькая `Ico Sphere`, которая используется как шаблон одной светящейся точки.

- `Camera`  
  Камера с `Track To`, смотрит в центр формы.

- `CameraTarget`  
  Empty-объект, в который направлена камера.

Также есть:

- материал `PointGlow`
- Geometry Nodes группа `PointCloudBlobNodes`

## Логика Geometry Nodes

Схема узлов делает следующее:

1. Берет позицию каждой точки исходной сферы.
2. Прогоняет ее через два `Noise Texture` с разной частотой.
3. Складывает два шума.
4. Превращает итог в величину смещения.
5. Смещает геометрию вдоль нормалей.
6. Преобразует вершины сетки в points.
7. Инстансит на каждую точку `DotInstance`.
8. Реализует инстансы и назначает emissive-материал.

Это даёт эффект “живой” точечной поверхности.

## Логика материала

Материал `PointGlow` построен так:

1. Берется мировая/локальная позиция точки.
2. Отделяется ось `Z`.
3. Значение `Z` нормализуется в диапазон `0..1`.
4. Добавляется немного `Noise`, чтобы цвет был не слишком стерильным.
5. Через `Color Ramp` строится градиент:
   - нижняя часть: magenta / pink
   - верхняя часть: cyan / blue
6. Цвет идет в `Emission`.

Именно это дает неоновую подсветку без внешних ламп.

## Почему форма выглядит как точечный blob

Основной визуальный эффект формируется тремя вещами:

- плотная UV Sphere с большим числом сегментов
- смещение поверхности шумом по нормали
- очень маленький инстанс-точка на каждой вершине

Если увеличить размер `DotInstance`, получится почти сплошная поверхность.  
Если уменьшить, точки станут слишком редкими и “воздушными”.  
Текущее значение подобрано как компромисс.

## Параметры, которые сильнее всего влияют на вид

### Форма

В `build_geometry_nodes()`:
...
Это управляет амплитудой и характером деформации.

### Плотность и размер точек

В `create_dot_instance()`:

- `radius=0.012`

В `create_blob_source()`:

- `segments=132`
- `ring_count=96`

Чем больше сегментов исходной сферы, тем больше точек на поверхности.

### Цвет и свечение

В `build_emission_material()`:
...
Полный запуск с созданием `.blend` и PNG:
...
Только собрать `.blend`, без рендера:
...
## Как повторить это вручную в Blender без Python

Если делать руками, последовательность такая:

1. Открыть пустую сцену.
2. Удалить дефолтные объекты.
3. Добавить `UV Sphere` с высокой детализацией.
4. Добавить маленькую `Ico Sphere`.
5. Создать Geometry Nodes modifier на большой сфере.
6. Собрать цепочку:
   - `Group Input`
   - `Position`
   - два `Noise Texture`
   - математика для смешивания
   - `Normal`
   - `Set Position`
   - `Mesh to Points`
   - `Object Info`
   ...
   - `Set Material`
   - `Group Output`
7. Создать материал с `Emission`.
8. Через `Geometry -> Position -> Separate XYZ -> Map Range -> Color Ramp` сделать цветовой переход.
9. Настроить Eevee и Bloom.
10. Поставить камеру чуть ниже центра и направить ее на объект.

## Полный код скрипта

Ниже сохранен полный актуальный код, из которого была собрана сцена.
...
Если хочешь получить другой характер формы, удобнее всего менять:
...
Если нужно, я могу отдельно сделать еще один файл:

- с версией под анимацию
- с версией под циклическое шевеление blob
- с версией под экспорт в `glb`
- с ручной схемой узлов по шагам именно в формате “нажми сюда в Blender”
